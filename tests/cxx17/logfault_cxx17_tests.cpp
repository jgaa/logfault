// tests/cxx17/test_logfault_cxx17.cpp
...
TEST(LogfaultCxx17, InfoLevelEmitsMessage) {
  ...
  std::string captured;
  {
      ...
      LFLOG_INFO << "Hello C++17 test";
  }

  // The fallback mode always prefixes a timestamp and "INFO"
  EXPECT_NE(captured.find("INFO"), std::string::npos);
  EXPECT_NE(captured.find("Hello C++17 test"), std::string::npos);
}

TEST(LogfaultCxx17, DebugLevelSuppressedWhenAboveThreshold) {
