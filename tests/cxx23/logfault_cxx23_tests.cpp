// tests/cxx20/test_logfault_cxx20.cpp
...
TEST(LogfaultCxx20, InfoLevelEmitsMessage) {
  ...
  std::string captured;
  {
      ...
      LFLOG_INFO << "Hello C++20 test";
  }

  // The fallback mode always prefixes a timestamp and "INFO"
  EXPECT_NE(captured.find("INFO"), std::string::npos);
  EXPECT_NE(captured.find("Hello C++20 test"), std::string::npos);
}

TEST(LogfaultCxx20, DebugLevelSuppressedWhenAboveThreshold) {
