// tests/cxx11/test_logfault_cxx11.cpp
...
TEST(LogfaultCxx11, InfoLevelEmitsMessage) {
  ...
  std::string captured;
  {
      ...
      LFLOG_INFO << "Hello C++11 test";
  }

  // The fallback mode always prefixes a timestamp and "INFO"
  EXPECT_NE(captured.find("INFO"), std::string::npos);
  EXPECT_NE(captured.find("Hello C++11 test"), std::string::npos);
}

TEST(LogfaultCxx11, DebugLevelSuppressedWhenAboveThreshold) {
